...
TaskHandle_t App_100ms_TaskHandler;

void Start_task(void *pvParameters)
{
    taskENTER_CRITICAL();

    vApplicationStartupHook();
    ...
    xTaskCreate((TaskFunction_t)App_Task_100ms,          // 任务函数
                (const char *)"App_Task_100ms",          // 任务名称
                (uint16_t)1024,                          // 任务堆栈大小
                (void *)NULL,                            // 传递给任务函数的参数
                (UBaseType_t)1,                          // 任务优先级
                (TaskHandle_t *)&App_100ms_TaskHandler); // 任务句柄
    
    printf("App_Task_100ms Create Success! \n");
